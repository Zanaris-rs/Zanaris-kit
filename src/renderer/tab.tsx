...
export default function Tab({
    ...
    return onSelect ? (
        <button type="button" role="tab" aria-selected={open} title={title} onClick={onSelect} style={BOX} className={skin}>
            {body}
        </button>
    ) : (
        <div role="tab" aria-selected={open} title={title} style={BOX} className={skin}>
            {body}
        </div>
    );
}
