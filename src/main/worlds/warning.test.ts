...
test('switching world names where you are going and where you are leaving', () => {
    assert.deepEqual(switchWarning({ kind: 'world', to: 2, from: 5 }), {
        ...
        detail: 'Zanaris Kit loads World 2 straight away, whether or not you are logged in. If you are in game on World 5, that logs you out.'
    });
});
...
test('the world being left is not the world being joined', () => {
    ...
    assert.ok(warning.detail.startsWith('Zanaris Kit loads World 41 straight away'));
...
test('switching to high detail names the world being reloaded', () => {
    assert.deepEqual(switchWarning({ kind: 'detail', to: 'high', world: 5 }), {
        ...
        detail: 'Zanaris Kit reloads World 5 at high detail straight away, whether or not you are logged in. If you are in game, that logs you out.'
    });
});
...
test('switching to low detail says low', () => {
    assert.deepEqual(switchWarning({ kind: 'detail', to: 'low', world: 1 }), {
        ...
        detail: 'Zanaris Kit reloads World 1 at low detail straight away, whether or not you are logged in. If you are in game, that logs you out.'
    });
});
